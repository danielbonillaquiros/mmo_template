    ...
    value: function setupEventListeners() {
      ...
      this.io.on('connection', function (socket) {
        ...
        socket.on('disconnect', function () {
          ...
          console.log(socket.id); // delete user data from the server

          delete _this2.playerLocations[socket.id]; // emit a message to all players to remove this player

          _this2.io.emit('player_disconnect', socket.id);
        });
        socket.on('newPlayer', function (token, frame) {
          try {
            ...
            if (process.env.BYPASS_AUTH !== 'ENABLED') {
              ...
              var decoded = _jsonwebtoken["default"].verify(token, process.env.JWT_SECRET); // get player's name
            ...
            socket.emit('current-items', _this2.items); // inform the other players of the new player that joined
              ...
              while (1) {
                switch (_context.prev = _context.next) {
                  case 0:
                    ...
                    // validate token, if valid send game information, else reject login
                    decoded = _jsonwebtoken["default"].verify(token, process.env.JWT_SECRET); // get player's name
        ...
        socket.on('player-dropped-item', function (itemId) {
          _this2.players[socket.id].removeItem(itemId);

          socket.emit('update-items', _this2.players[socket.id]);
          socket.broadcast.emit('update-players-items', socket.id, _this2.players[socket.id]);
        });
        socket.on('pickup-item', function (itemId) {
          // update the spawner
          if (_this2.items[itemId]) {
            if (_this2.players[socket.id].canPickupItem()) {
              ...
              socket.emit('update-items', _this2.players[socket.id]);
              socket.broadcast.emit('update-players-items', socket.id, _this2.players[socket.id]); // removing the item
        ...
        socket.on('pickUpChest', function (chestId) {
          ...
          if (_this2.chests[chestId]) {
            ...
            socket.broadcast.emit('update-players-score', socket.id, _this2.players[socket.id].gold); // removing the chest
        ...
        socket.on('monsterAttacked', function (monsterId) {
          ...
          if (_this2.monsters[monsterId]) {
            ...
            var playerAttackValue = _this2.players[socket.id].attack; // subtract health from monster model

            _this2.monsters[monsterId].loseHealth(playerAttackValue); // check the monster's health, if dead, remove it
            ...
            if (_this2.monsters[monsterId].health <= 0) {
              ...
              _this2.io.emit('updatePlayerHealth', socket.id, _this2.players[socket.id].health); // update the monster's health
        ...
        socket.on('player_attacked', function (attackedPlayerId) {
          if (_this2.players[attackedPlayerId]) {
            ...
            _this2.players[attackedPlayerId].playerAttacked(playerAttackValue); // check attacked player's health, if dead send gold to other player
            ...
            if (_this2.players[attackedPlayerId].health <= 0) {
              ...
              socket.emit('updateScore', _this2.players[socket.id].gold); // reset the attacked player's gold
